...
import parsley.Parsley.{LazyParsley, pos, select, sequence}
import parsley.implicits.Map3
...
case class Assignment(
  ...
  override def check(symbolTable: SymbolTable)(implicit errors : mutable.ListBuffer[Error]): Unit = {
    ...
    if (assignmentLeft.getType(symbolTable)
    ...
    symbolTable.dictionary.updateWith(identifier.identifier)({
      ...
      case None => {
        ...
        if (identType.sameTypes(assignmentRight, symbolTable)) {
          ...
          errors += DefaultError("Invalid types in identifier assignment. Got: " +
            identType.getType(symbolTable) + assignmentRight.getType(symbolTable), pos)
          None
        }
      }
    })
...
case class If(
  ...
  override def check(symbolTable: SymbolTable)(implicit errors : mutable.ListBuffer[Error]): Unit = {
    ...
    if (condition.getType(symbolTable).unifies(BooleanType())) {
      ...
      errors += DefaultError("If condition does not evaluate to Boolean. Got " + condition.getType(symbolTable) +
       " in " + condition.toString, pos)
    }
    List.empty
  }
}
...
object If {
  ...
  def apply(expr: => Parsley[Expression], body: => Parsley[Statement], `else` : => Parsley[Statement]): Parsley[If] =
    pos <**> (expr, body, `else`).map((c, b, e) => (pos: (Int, Int)) => If(c, b, e)(pos))
}
