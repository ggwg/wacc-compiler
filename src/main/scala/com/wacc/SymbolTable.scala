...
class SymbolTable(parentSymbolTable: SymbolTable) {
  ...
  var dictionary = Map[String, (PairElementType, ASTNodeVoid)]()
...
//  }
  ...
  def lookup(name: String) : Option[(PairElementType, ASTNodeVoid)] = {
    return dictionary.get(name)
  }

  /* Looks up all the symbol tables */
  def lookupAll(varName: String) : Option[(PairElementType, ASTNodeVoid)] = {
